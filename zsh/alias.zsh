...
# 清空系统一小时以前的所有日志
alias journal-clean='sudo journalctl --rotate --vacuum-time=1h'

function @zeta:-init-alias() {
  local _ycmd_ _yarg_ _yvar_ _ymsg_ _ytmp_  _yidx_ _ycnt_ _ynum_ _yval_ _flag_
  local _zcmd_ _zarg_ _zvar_ _zmsg_ _ztmp_  _zidx_ _zcnt_ _znum_ _zval_ _isok_
...
@zeta:no-cmd zff && alias zff='find . -type f -name'
@zeta:no-cmd zfd && alias zfd='find . -type d -name'
...
function ls-dot-files() {
  ...
  [[ ! -d "${folder}" ]] && return 1
  # 引号之外的 .* 用于 Shell Glob File Name Match
  ls -hld "${folder}"/.*(N) 2> /dev/null
}
...
@zeta:no-cmd now && alias now=timestamp-iso-8601-now
...
function desktop-alert-when-done() {
  [[ $# -eq 0 || -z "$1" ]] && {
    @zeta:note desktop-alert-when-done sleep 5; return 1
  } # NOTE $ notify-send "Summary" "Body"; man libnotify-bin
  @zeta:no-cmd notify-send && return 2 # $ apt show libnotify-bin
  local _cmd_=$1; shift; @zeta:no-cmd ${_cmd_} && return 1
...
function extract() {
  if [[ $# -eq 0 || -z "$1" ]]; then
    @zeta:note extract path/to/tarball; return
  fi
  [[ ! -f "$1" ]] && return 1
...
alias is-binnum='@zeta:is-binnum'
alias is-octnum='@zeta:is-octnum'
alias is-decnum='@zeta:is-decnum'
alias is-hexnum='@zeta:is-hexnum'
alias is-number='@zeta:is-number'

function repo-head-hash() {
  @zeta:no-cmd git && return
  if @zeta:is-inside-git-repo; then
