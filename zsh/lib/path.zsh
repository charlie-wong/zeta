...
function @zeta:-cleanup-path() {
  # NOTE `path` is special, space split list for zsh
  local _path_ _PATH_="${PATH// /␟}" # 保持可能的空格
  _PATH_=( ${=_PATH_//:/ } ) # 强制进行 IFS 字符分隔

  # https://www.freedesktop.org/wiki/Software/systemd/TheCaseForTheUsrMerge
  unset -v PATH; export PATH="/usr/bin:/usr/sbin"
  ...
  # 调用 SHELL 外程序执行速度较慢 $(echo "${PATH}" | sed 's/:/\t/g')
