...
if [ -x /usr/bin/dircolors ]; then
  if [ -r "${ZETA_DIR}/zsh/extra/dircolors" ]; then
    eval "$(dircolors -b "${ZETA_DIR}/zsh/extra/dircolors")"
  else
    eval "$(dircolors -b)"
  fi
fi
...
function @zeta:-colored() {
  ...
  RbOut="${(P)_RbCNS_[${RbIdx}]}${_RbMsg_}\e[0m"
  (( RbIdx++, RbIdx = RbIdx % _RbMax_ ))
}

function @zeta:rainbow() {
  local xTitle="$1" xMsg  zFB zLB zIt; shift
  local y1='\e[1;5;90m' y2='\e[0m' RbIdx RbOut
  for zIt in "$@"; do
    zFB="${zIt:0:1}"; zLB="${zIt:${#zIt}-1:1}"
    case "${zFB}${zLB}" in
      "[]") @zeta:-colored "${zIt:1:${#zIt}-2}"; xMsg+=" ${y1}[${y2}${RbOut}${y1}]${y2}" ;;
      "<>") @zeta:-colored "${zIt:1:${#zIt}-2}"; xMsg+=" ${y1}<${y2}${RbOut}${y1}>${y2}" ;;
      "()") @zeta:-colored "${zIt:1:${#zIt}-2}"; xMsg+=" ${y1}(${y2}${RbOut}${y1})${y2}" ;;
      "{}") @zeta:-colored "${zIt:1:${#zIt}-2}"; xMsg+=" ${y1}{${y2}${RbOut}${y1}}${y2}" ;;
         *) @zeta:-colored "${zIt}"; xMsg+=" ${RbOut}" ;;
    esac
  done; printf "${xTitle}${xMsg}\n"
}

function zeta-color-preview() {
  echo
